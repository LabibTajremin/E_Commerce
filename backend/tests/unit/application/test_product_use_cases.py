...
from src.domain.exceptions import EntityAlreadyExistsError, EntityNotFoundError, ValidationError
...
async def test_create_product_generates_unique_slug() -> None:
    ...
    tenant_id = uuid4()
    use_case = CreateProductUseCase(products, categories)
...
async def test_create_product_rejects_unknown_category() -> None:
    ...
    with pytest.raises(EntityNotFoundError):
        await CreateProductUseCase(products, categories).execute(
            CreateProductInput(
