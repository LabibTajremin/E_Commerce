...
class ProductRepository(Protocol):
    ...
    async def get_by_id_for_update(self, tenant_id: UUID, product_id: UUID) -> Product | None:
        """Locks the row (SELECT ... FOR UPDATE) for the duration of the
        current transaction — used by checkout to serialize stock decrements
        and prevent oversell under concurrent orders on the same product."""
        ...
